...
import {
  ...
  MAX_PROJECT_FILE_REFERENCE_LABEL_CHARS,
...
function assertHighlight(
  ...
  for (const entry of candidate.tocPath) {
    if (
      ...
      || !entry.title
      || entry.key.length > MAX_PROJECT_FILE_REFERENCE_LABEL_CHARS
      || entry.title.length > MAX_PROJECT_FILE_REFERENCE_LABEL_CHARS
      || (entry.href !== undefined && (
        typeof entry.href !== 'string'
        || entry.href.length > MAX_PROJECT_FILE_REFERENCE_LABEL_CHARS
      ))
      || !Array.isArray(entry.orderPath)
      || entry.orderPath.length > MAX_EPUB_TOC_PATH_DEPTH
...
function statePath(
  ...
  const directory = join(getProjectPath(workspaceRootPath, project.config.slug), 'reader-state', 'v1', 'epub')
  return {
    directory,
...
async function readStateFile(
  ...
  if (!isStoredState(parsed, identity)) {
    await quarantineCorruptState(path)
    return null
  }
  return parsed
...
export class EpubStateStore {
  ...
  async get(
    ...
    return this.withLock(
      resolved.path,
      () => readStateFile(resolved.path, identity),
    )
  }
  ...
  async apply(
    ...
    return this.withLock(resolved.path, async () => {
      ...
      let canonicalHighlight: EpubHighlightV1 | undefined
      ...
      return {
        ...
        applied: true,
        ...(canonicalHighlight ? { canonicalHighlight } : {}),
      }
    })
  }
