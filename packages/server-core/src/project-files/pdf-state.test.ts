...
function persistedStatePath(): string {
  const directory = join(getProjectPath(root, projectSlug), 'reader-state', 'v1', 'pdf')
...
describe('PdfStateStore', () => {
  ...
  it('serializes reads behind an in-flight state write', async () => {
    ...
    const store = new PdfStateStore(
      ...
      async (directory, path, state) => {
        markWriteStarted()
        await writeReleased
        await writePdfStateFile(directory, path, state)
      },
    )
  ...
  it('rejects a mutation before it exceeds the state byte budget', async () => {
    ...
    })).rejects.toThrow('PDF_STATE_TOO_LARGE')
  })

  it('quarantines corrupt and oversized persisted state', async () => {
    const store = new PdfStateStore(() => 100)
