...
import { CONFIG_DIR, getWorkspaceByNameOrId, getGitBashPath, setGitBashPath, clearGitBashPath } from '@craft-agent/shared/config'
...
function parseInternalDeepLink(
  parsed: URL,
  scheme: string,
): ParsedInternalDeepLink | null {
  if (parsed.protocol !== `${scheme.replace(/:$/u, '').toLowerCase()}:`) return null
...
export function registerSystemCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  // Shell operations - open URL in external browser (or handle product deep links internally)
  server.handle(RPC_CHANNELS.shell.OPEN_URL, async (ctx, url: string) => {
    deps.platform.logger.info('[OPEN_URL] Received request:', url)
    try {
      const deepLinkScheme = process.env.CRAFT_DEEPLINK_SCHEME || 'craftagents'
      const classification = classifyExternalUrl(url, deepLinkScheme)
      ...
      if (classification.kind === 'internal-deeplink') {
        const deepLink = parseInternalDeepLink(parsed, deepLinkScheme)
