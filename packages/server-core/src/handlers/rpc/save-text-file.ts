...
export function validateSaveTextFileRequest(value: unknown): SaveTextFileRequest {
  ...
  const filenameBytes = Buffer.byteLength(suggestedName, 'utf8')
  const isBasename = (
  ...
  if (
    ...
    || !isBasename
    || filenameBytes > MAX_SUGGESTED_FILENAME_BYTES
  ...
  // Construct a fresh value so unknown fields (especially a caller-supplied
  // output path) are never forwarded to the local client.
  return {
    suggestedName,
...
export function registerSaveTextFileHandlers(server: RpcServer): void {
  server.handle(
    ...
    async (ctx, value: unknown): Promise<SaveTextFileResponse> => {
      const request = validateSaveTextFileRequest(value)
      return requestClientSaveTextFile(server, ctx.clientId, request)
    },
  )
}
