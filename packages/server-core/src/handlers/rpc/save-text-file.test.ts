...
describe('save text file RPC', () => {
  ...
  it('rejects paths, unsafe basenames, and oversized text', () => {
    for (const suggestedName of [
      '../outside.md', 'folder/file.md', 'folder\\file.md', '/absolute.md',
      'C:\\absolute.md', '.', '..', ' trailing.md', 'trailing.md.',
      'bad:name.md', 'CON.md', 'nul\u0000.md',
    ]) {
      expect(() => validateSaveTextFileRequest({ suggestedName, content: '' }))
        .toThrow('Invalid suggested filename')
    }
    expect(() => validateSaveTextFileRequest({
      suggestedName: 'draft.md',
      content: 'a'.repeat(MAX_SAVED_TEXT_BYTES + 1),
