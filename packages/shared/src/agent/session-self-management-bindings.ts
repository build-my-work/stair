 ...
 *
 * Attaches session-scoped callback properties to a SessionToolContext using
 ...
 *
 * Defines lazy getters for the registry-backed session tools.
 *
...
export function attachSessionSelfManagementBindings(
  ...
  Object.defineProperty(context, 'readMindmap', {
    get() {
      return getSessionScopedToolCallbacks(sessionId)?.readMindmapFn;
    },
    configurable: true,
    enumerable: true,
  });

  Object.defineProperty(context, 'updateMindmap', {
    get() {
      return getSessionScopedToolCallbacks(sessionId)?.updateMindmapFn;
    },
    configurable: true,
    enumerable: true,
  });
