...
const AUTO_UPDATE_DISABLED = process.env.CRAFT_DISABLE_AUTO_UPDATE === '1'
...
export async function checkForUpdates(options: CheckOptions = {}): Promise<UpdateInfo> {
  if (AUTO_UPDATE_DISABLED) {
    mainLog.info('[auto-update] Disabled for this product profile')
...
export async function installUpdate(): Promise<void> {
  ...
  try {
    ...
    broadcastUpdateInfo()
    autoUpdateLog.error('beforeUpdateInstall cleanup hook failed; update install cancelled', err)
    throw err
  }
