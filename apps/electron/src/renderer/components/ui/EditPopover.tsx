...
const EDIT_CONFIGS: Record<EditContextKey, (location: string) => EditConfig> = {
  ...
  'default-permissions': (location) => ({
    context: {
      ...
      context:
        `The user is editing app-level default permissions (${location}). ` +
  ...
  'preferences-notes': (location) => ({
    context: {
      ...
      context:
        `The user is editing the notes field in their preferences (${location}). ` +
  ...
  'add-source': (location) => ({
    context: {
      ...
      context:
        ...
        'Follow the bundled sources documentation. ' +
  ...
  'add-skill': (location) => ({
    context: {
      ...
      context:
        ...
        'Follow the bundled skills documentation. ' +
  ...
  'edit-labels': (location) => ({
    context: {
      ...
      context:
        ...
        'Read the bundled labels documentation for the full format reference. ' +
        'Confirm clearly when done.',
    },
  ...
  'edit-tool-icons': (location) => ({
    context: {
      ...
      context:
        ...
        'The file is the configured tool-icons.json. Icon image files live in the same directory. ' +
        ...
        'Read the bundled tool-icons documentation for the full format reference. ' +
  ...
  'automation-config': (location) => ({
    context: {
      ...
      context:
        ...
        'Read the bundled automations documentation for the full format reference. ' +
...
export function EditPopover({
  ...
  const { defaultProjectId, onCreateSession, onSendMessage, onRespondToPermission, onRespondToCredential } = useAppShellContext()
  ...
  const stubSession = useMemo((): Session | null => {
  ...
  }, [defaultProjectId, workspace?.id, workspace?.name])
  ...
  const isProcessing = displaySession?.isProcessing ?? false
  ...
  const handleLegacySendMessage = useCallback((message: string) => {
    ...
    const url = buildProductDeepLink(
      `action/new-session?input=${encodedInput}&send=true&mode=${permissionMode}&badges=${encodedBadges}${workdirParam}${modelParam}${systemPromptParam}`,
    )
