 ...
 *
 * It observes real user text selections and sends a bounded snapshot to the
 * main process. Nothing is exposed into the page's main world.
 */
...
function textBeforeBoundary(range: Range, limit: number): string {
  ...
  if (!root || boundary.nodeType !== Node.TEXT_NODE || !root.contains(boundary)) return ''

  const chunks: string[] = [
    (boundary.nodeValue ?? '').slice(0, range.startOffset),
  ]
...
function textAfterBoundary(range: Range, limit: number): string {
  ...
  const chunks: string[] = [
    (boundary.nodeValue ?? '').slice(range.endOffset),
  ]
  let length = chunks[0].length
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT)
  walker.currentNode = boundary
...
function captureSelection(expectedToken: number): void {
  ...
  const payload: BrowserSelectionCapturePayload = {
    ...
    prefix: textBeforeBoundary(range, BROWSER_SELECTION_LIMITS.context) || undefined,
    suffix: truncated
      ? undefined
      : textAfterBoundary(range, BROWSER_SELECTION_LIMITS.context) || undefined,
    rect: {
      x: focusRect.x,
      y: focusRect.y,
...
if (isSupportedMainFrame()) {
  ...
  window.addEventListener('pointerup', (event) => {
    if (!event.isTrusted || !event.isPrimary || isEditableNode(event.target as Node | null)) return
    scheduleCapture()
  }, true)
